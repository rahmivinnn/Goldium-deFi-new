  ...
  return (
    <header
      className={`navbar-optimized fixed top-0 left-0 right-0 z-50 transition-all duration-300 will-change-auto transform-gpu ${
        isScrolled ? "bg-black/95 backdrop-blur-xl shadow-2xl shadow-yellow-500/20 border-b border-yellow-500/20" : "bg-gradient-to-b from-black/50 to-transparent backdrop-blur-sm"
      }`}
          ...
          <nav className="hidden md:flex items-center space-x-6">
            {navItems.map((item) => (
              <Link 
                key={item.name} 
                href={item.href} 
                className="navbar-link optimized-text text-white hover:text-yellow-300 transition-all duration-200 px-4 py-2 rounded-lg hover:bg-gradient-to-r hover:from-yellow-500/20 hover:to-amber-500/20 hover:shadow-lg hover:shadow-yellow-500/20 cursor-pointer select-none gpu-accelerated transform hover:scale-105 border border-transparent hover:border-yellow-500/30"
              >
                {item.name}
              </Link>
            ))}
