...
export default function ViralHeroSection() {
  ...
  const [playersOnline, setPlayersOnline] = useState(3247)
  const [gamesPlayed, setGamesPlayed] = useState(847392)
  const [tokensEarned, setTokensEarned] = useState(2847392)
  ...
  useEffect(() => {
    const statsInterval = setInterval(() => {
      ...
      setGamesPlayed(prev => prev + Math.floor(Math.random() * 10))
      setTokensEarned(prev => prev + Math.floor(Math.random() * 500))
    }, 3000)
            ...
            <div className="space-y-8">
              ...
              >
                <Button
                  ...
                  className="bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 text-white font-bold text-lg px-8 py-4 rounded-xl shadow-lg hover:shadow-purple-500/25 transition-all duration-300 transform hover:scale-105"
                >
                  <Sword className="w-5 h-5 mr-2" />
                  {connected ? 'Play 3D Game' : 'Connect & Play'}
                ...
                <Button
                  ...
                  className="border-2 border-white/20 text-white hover:bg-white/10 font-semibold text-lg px-8 py-4 rounded-xl"
                >
                  <Joystick className="w-5 h-5 mr-2" />
                  {connected ? '2D Games' : 'Connect for 2D'}
                </Button>
              </motion.div>
